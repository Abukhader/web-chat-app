<?php
    ...
    if(!empty($fname) && !empty($lname) && !empty($email) && !empty($password)) {
        ...
        //check user email is vaild or not
        if(filter_var($email, FILTER_VALIDATE_EMAIL)){
            ...
            if(mysqli_num_rows($sql) > 0){
                ...
                if(isset($_FILES['image'])){
                    ...
                    $tmp_name = $_FILES['image']['tmp_name'];//this tempoary name is used to save file in our folder
                   ...
                   $extensions = ['png', 'jpeg', 'jpg']; //valid img 
                   if(in_array($img_ext, $extensions) === true){
                       ...
                       $time = time(); //this will return us current time..
                        ...
                        if(move_uploaded_file($tmp_name, "images/".$new_img_name)){ //if user upload to our folder successfuly
                            $status = "Active now";
